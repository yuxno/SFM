...
# K = np.array ([
#     [107.47451339, 0, 121.70781587],
#     [0, 105.47125235, 150.95623602],
#     [0, 0, 1]
# ])
...
def PnP(X, p, K, d, p_0, initial):
    ...
    print(f"3D points (X): {X.shape}")
    print(f"2D points (p): {p.shape}")
    
    if X.shape[0] < 4 or p.shape[0] < 4:
        print(f"Insufficient points for solvePnPRansac: {X.shape[0]} 3D points, {p.shape[0]} 2D points")
...
def find_features(img0, img1):
    ...
    print(f"Number of keypoints detected: {len(kp0), len(kp1)}")
...
if img0 is None or img1 is None:
    print(f"Error reading images {images[i]} or {images[i + 1]}")


pts0, pts1 = find_features(img0, img1)
if len(pts0) == 0 or len(pts1) == 0:
    print(f"Not enough features found between images {images[i]} and {images[i + 1]}")
